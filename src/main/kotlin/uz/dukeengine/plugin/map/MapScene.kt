 ...
 */
object MapScenes {
    private const val LAYERED = "uz.dukeengine.core.thing.Layered"
    ...
    fun of(map: MapModel): MapScene? {
        ...
        val scene = mapOf(
            "cell" to 10,
            "levelHeight" to levelHeight(block, blocks),
            "cells" to map.rows,
            "solid" to map.solid,
    ...
    /**
     * How high a storey stands: the map's own `LevelHeight` where it says one, else the world's.
     *
     * <p>The map first, as the engine lays it — a map that says its own height is laid at it, and the world's is
     * what a map is laid at when it has not said (see `GameLogic.setPathGrid`). Both records may be `Layered`, so
     * taking whichever came first in the file list drew a map at whichever height it happened to find.
     */
    private fun levelHeight(map: DukeBlock, blocks: List<DukeBlock>): Float {
        val own = if (isLayered(map)) number(map, "LevelHeight") ?: 0f else 0f
        if (own > 0f) {
            return own
        }
        // Not merely "another Layered block": every other map in the project is one too, and the first of them
        // to come past would decide this map's height. The world is the Layered block that is not a map.
        val world = blocks.firstOrNull { isLayered(it) && !isMap(it) } ?: return 0f
        return number(world, "LevelHeight") ?: 0f
    }

    private fun isLayered(block: DukeBlock) =
        DukeRecords.recordOf(block)?.let { record -> record.interfaces.any { it.qualifiedName == LAYERED } } == true

    private fun isMap(block: DukeBlock) =
        DukeRecords.recordOf(block)?.recordComponents?.any { it.hasAnnotation(GRID) } == true
